...
from utils.briarmbg import BriaRMBG
...
def process_relight(input_fg, input_bg, prompt, image_width, image_height, num_samples, seed, steps, a_prompt, n_prompt, cfg, highres_scale, highres_denoise, bg_source):
    ...
    out=results + extra_images

    for i, img_array in enumerate(out):
        img_array = cv2.cvtColor(img_array, cv2.COLOR_BGR2RGB)
        img = Image.fromarray(img_array)
        file_path = "images/RelightOutput_"+f"image_{i + 1}.png"
        img.save(file_path)
...
def apply_relight(input_fg, input_bg):

    fg=cv2.imread(input_fg)
    bg=cv2.imread(input_bg)

    print(fg.shape)
    print(bg.shape)

    prompt = "realistic lighting"
    bg_source = BGSource.UPLOAD.value
    num_samples = 1
    seed = 123
    image_width=bg.shape[1]
    image_height = bg.shape[0]
    steps = 50
    cfg = 1.5
    highres_scale = 1.5
    highres_denoise = 0.5
    ...
    process_relight(fg, bg, prompt, image_width, image_height, num_samples, seed, steps, a_prompt, n_prompt, cfg, highres_scale, highres_denoise, bg_source)
